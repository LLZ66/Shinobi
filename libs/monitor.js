...
module.exports = function(s,config,lang){
    ...
    s.cleanMonitorObjectForDatabase = function(dirtyMonitor){
        var cleanMonitor = {}
        var acceptedFields = ['mid','ke','name','shto','shfr','details','type','ext','protocol','host','path','port','fps','mode','width','height','x','y']
    ...
    s.addOrEditMonitor = function(form,callback,user){
        ...
        s.sqlQuery('SELECT * FROM Monitors WHERE ke=? AND mid=?',[form.ke,form.mid],function(er,r){
            ...
            if(r&&r[0]){
                ...
                console.log(monitorQueryValues)
                console.log(monitorQuery)
                s.userLog(form,{type:'Monitor Updated',msg:'by user : '+user.uid+form.position})
